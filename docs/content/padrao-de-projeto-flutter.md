...
- **Economia de recursos**: instanciação **lazy** e **dispose** automático do estado de tela. Os limites disso estão na [seção 25](#25-economia-de-recursos-o-que-é-real-e-o-que-não-é).

> **Honestidade sobre "grande porte".** Esta arquitetura escala bem **sob uma disciplina** (a regra de ouro do `get_it`, [seção 15](#15-injeção-de-dependências-quem-localiza-quem-injeta)) e com a noção correta do que o dispose cobre. As [seções 16](#24-robustez-e-escala-o-que-é-estrutural-e-o-que-depende-de-disciplina) e [17](#25-economia-de-recursos-o-que-é-real-e-o-que-não-é) são francas sobre os limites, porque "robusto" sem ressalva é marketing, não engenharia.
...
**Imutabilidade.** Entidades e estados são **imutáveis** e estendem `Equatable` (igualdade por valor). Sem `freezed` (que exige `build_runner`), o `copyWith` é escrito à mão, um trade-off consciente cujos detalhes e armadilhas estão na [seção 11](#11-imutabilidade-copywith-manual-e-seus-limites).
...
**Sem `build_runner`.** Nenhum `.g.dart`. Tudo roda com as ferramentas padrão do Dart e do Flutter.
...
| [`equatable`](https://pub.dev/packages/equatable) | **Igualdade por valor** | Faz o dedupe de rebuild do bloc funcionar (ou seja, não reconstruir a tela quando o estado é igual); estados e entidades comparáveis sem geração de código. **Essencial** (ver [seção 8](#8-domínio-e-dados-a-regra-que-não-pode-vazar)). |
...
- **Dispose automático**: o `BlocProvider` fecha o cubit ao sair da árvore ([seção 25](#25-economia-de-recursos-o-que-é-real-e-o-que-não-é)).
...
A saída idiomática no Flutter, sem build_runner, é o **`ValueGetter`**. Ele é só um typedef do `flutter/foundation`: `typedef ValueGetter<T> = T Function()`. O truque é embrulhar o valor numa função. A **presença da função** responde "o caller passou algo?", e **chamá-la** entrega o valor, que pode muito bem ser `null`:
...
- **Sem geração de código, cada campo novo são três lugares.** Adicionou um campo na entidade? Você mexe no construtor, no `props` e no `copyWith`. Esquecer o `copyWith` é o pior dos três, porque ele simplesmente para de copiar o campo novo e nada reclama. É o "custo recorrente sem build_runner" da [seção 24](#24-robustez-e-escala-o-que-é-estrutural-e-o-que-depende-de-disciplina), visto de perto.
...
Custo zero, sem build_runner. É a mesma mecânica que faz o `Widget` do Flutter avisar quando você esquece um `final`.
...
Repare como isso espelha a estratégia de erro em duas camadas da [seção 9](#9-erros-em-duas-camadas-fpdart-para-os-previstos). O artigo já trata o deep link **malformado por página** (o `int.tryParse` que cai no `_InvalidProductScreen`, [seção 7.6](#76-apresentação-estado-e-página-com-fpdart-no-cubit)); o `onException` é a rede no nível do app, para a rota que nem chega a casar. Duas camadas de robustez de deep link, mesma filosofia.
...
A [seção 13](#13-navegação-com-go_router-qual-método-usar) avisou para não espalhar string mágica. Aqui está o padrão completo, sem code-gen: cada módulo é dono dos seus nomes de rota e expõe **helpers** que embrulham a navegação com os params certos. É o `CatalogRoutes` da [seção 7.7](#77-fiação-rotas-injeção-e-o-barrel-público), agora sem string solta:
...
É aqui (e só aqui) que existe "dispose modularizado" de serviços ([seção 25](#25-economia-de-recursos-o-que-é-real-e-o-que-não-é)).
...
### Config por arquivo, sem geração de código

Para não chumbar a config no código, o jeito moderno e sem build_runner é o `--dart-define-from-file`, que injeta valores em tempo de compilação, lidos por `String.fromEnvironment`:
...
## 23. Performance: medindo com o Flutter DevTools
...
- **Memory.** Mostra o heap (a memória alocada) e ajuda a flagrar vazamento. É aqui que dá para conferir uma promessa do guia: o dispose automático do estado de tela ([seção 25](#25-economia-de-recursos-o-que-é-real-e-o-que-não-é)). Abra uma tela, volte, e veja o cubit sair da memória.
...
## 24. Robustez e escala: o que é estrutural e o que depende de disciplina
...
## 25. Economia de recursos: o que é real e o que não é
...
| **Testes (`bloc_test` + `mocktail`)** | Mock injetado por construtor, sem build_runner; estados conferidos por valor |
...
| **Internacionalização (`intl` + ARB)** | texto fora do código; exceção consciente ao sem-codegen |
...
## 26. Versões de referência (junho de 2026)
...
## 27. Próximos capítulos (sem `build_runner`)
