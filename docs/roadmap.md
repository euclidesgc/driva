...
- `[ ]` **42. Identidade visual: o ícone do app deixa de ser o do Flutter.** _(pedido do humano, 2026-08-16; 0-dep, fica para depois)_ `web/icons/` ainda tem os quatro PNGs do template (`Icon-192`, `Icon-512` e os dois `maskable`), e o `favicon.png` também. **O item 41 tornou isso visível:** a F2c fez do `/preview` um alvo instalável, então o ícone do Flutter agora mora na **tela inicial do celular** de quem usa — não só na aba do navegador. **A direção dada pelo humano:** letras estilizadas, o **D** e o **B** de **DrivaBuilder**, algo artístico, moderno, com aspecto profissional. **Alvos a trocar:** os quatro ícones, o `favicon.png` e — atenção — os **dois** manifests (`manifest.json` e `preview_manifest.json`), que hoje apontam para o mesmo conjunto; decidir se editor e preview compartilham o ícone ou se diferenciam na tela inicial, já que a D31 os fez **dois** apps instaláveis. ⚠️ **Pendente do humano antes de abrir:** o nome do produto — os manifests dizem "driva Editor"/"driva Preview" e o pedido diz "DrivaBuilder". Se o nome mudou, é mais do que ícone.
