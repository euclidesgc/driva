...
- `[ ]` **9f. 🔴 BLOQUEADOR — Projetos não funciona em homologação.** _(descoberto 2026-07-10; o E2E do 9d foi só de API em localhost, nunca abriu a UI nem bateu no hml — ver memória `e2e-precisa-exercitar-de-verdade`)_ Duas quebras independentes: **(a) diálogo "Novo projeto" crashava** na tela (`Spacer` dentro do `OverflowBar` do `AlertDialog` → `RenderErrorBox` cinza em release) — **corrigido** (PR #46, com teste de regressão); **(b) backend do 9d nunca subiu no hml** — `GET /v1/projects` → **404** (`Cannot GET`, container anterior no ar; `/health` e `/v1/contents` respondem), enquanto o código do backend tem a rota (`@Controller('projects')` + prefixo global `v1`) e as migrações fazem seed+backfill+NOT NULL na ordem correta. **Falta:** redeployar o backend no hml (Coolify) e confirmar `/v1/projects` respondendo + migrações aplicadas; **e** instrumentar o E2E que faltou (prints CDP abrindo criar-projeto + checagem de API **contra o hml**, não localhost). Bônus verificado nesta rodada: o ícone de tema invisível foi resolvido de vez (PR #44/#45, `--no-tree-shake-icons` + fontes `no-cache`).
