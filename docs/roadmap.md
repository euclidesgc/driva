...
- `[-]` **42. Identidade visual: o ícone do app deixa de ser o do Flutter, e o nome próprio aparece.** _(pedido do humano, 2026-08-16; 0-dep)_ **Quick win priorizado pelo humano em 2026-08-16, para logo após o item 41 fechar.**
  ...
  - `[-]` **O ícone — 2 candidatos gerados, aguardando escolha final.** `web/icons/` ainda tem os quatro PNGs do template Flutter (`Icon-192`, `Icon-512`, dois `maskable`) e o `favicon.png` também. Direção do humano: monograma **D+B** de **Driva Builder**, na **Space Grotesk Bold** (a mesma fonte do wordmark do topo do editor — a primeira rodada usou um traço desenhado à parte e foi recusada por isso). **Decidido (2026-08-16): ícone único** para Driva Editor e Driva Preview — os dois manifests passam a apontar para o mesmo conjunto de PNGs quando os finais entrarem. **Próxima ação:** humano escolhe entre Concepto A (selo escuro, glifo em dois tons) e Concepto B (fundo dividido laranja/tinta), ou pede ajuste — decisão estética, não entra direto como código.
...
- `[ ]` **44. Título do seletor de cor vaza para fora do modal.** _(relato do uso real, 2026-08-16; 0-dep)_ No `ColorEditor` (`.../editor/widgets/prop_field/color_editor.dart`), o `heading: const Text('Selecionar cor')` passado ao `ColorPicker` do `flex_color_picker` estoura a largura do modal. Correção provável, a confirmar no discovery: remover o `heading` (o diálogo já deixa claro o que é) em vez de tentar encaixar o texto.
