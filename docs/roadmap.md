...
- `[ ]` **42. Identidade visual: o ícone do app deixa de ser o do Flutter, e o nome próprio aparece.** _(pedido do humano, 2026-08-16; 0-dep)_ **Quick win priorizado pelo humano em 2026-08-16, para logo após o item 41 fechar.** A metade de texto (ver nota abaixo, "não depende de arte") entra direto; o ícone entra como **rascunho (1-2 candidatos de monograma D+B) para aprovação do humano antes** de gerar os PNGs finais — decisão estética, não técnica. `web/icons/` ainda tem os quatro PNGs do template (`Icon-192`, `Icon-512` e os dois `maskable`), e o `favicon.png` também. **O item 41 tornou isso visível:** a F2c fez do `/preview` um alvo instalável, então o ícone do Flutter agora mora na **tela inicial do celular** de quem usa — não só na aba do navegador.
  ...
  - **O ícone, com a direção do humano:** letras estilizadas, o **D** e o **B** de **Driva Builder**, algo artístico, moderno, com aspecto profissional. Trocar os quatro PNGs e o `favicon.png`. Os **dois** manifests hoje apontam para o mesmo conjunto — decidir se os dois alvos instaláveis compartilham o ícone ou se diferenciam na tela inicial, já que a D31 os fez dois apps instaláveis lado a lado.
  - **O que carrega nome hoje, e está errado:** `index.html` ainda é o template do Flutter — `<title>driva_editor</title>`, `apple-mobile-web-app-title: "driva_editor"` e `description: "A new Flutter project."`. **É o que aparece na aba do navegador de todo mundo agora**, e no iOS é o nome sugerido ao adicionar à tela inicial. Nos manifests, `name` é "driva Editor"/"driva Preview" com o `d` minúsculo, e o `short_name` do editor é só "driva". _Esta parte não depende de arte nenhuma e pode andar antes do ícone._
