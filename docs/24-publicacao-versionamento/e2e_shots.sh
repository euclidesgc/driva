#!/usr/bin/env bash
# E2E — item 24 (publicação e versionamento) / captura VISUAL automatizada
...
# hml o ciclo inteiro — publicar, ver o selo da lista mudar, abrir o histórico,
# restaurar, republicar o rollback, despublicar — e **assere o estado por API
# depois de cada passo**. Inclui, de propósito, os DOIS MODOS DE FALHA que o
# item introduziu (`publishFailed` e `restoreFailed`): o driver corta a rota por
# CDP e prova que falhar produz um estado visualmente distinto de dar certo.
#
# Aponta para o hml REAL (o mesmo artefato que o Coolify serve), nunca localhost
# — lição registrada do item 9g.
#
# Uso:
#   docs/24-publicacao-versionamento/e2e_shots.sh          # próxima rodada livre
#   docs/24-publicacao-versionamento/e2e_shots.sh 01       # em evidencias/rodada_01/
#
# Env (default = homologação):
#   WEB_BASE=https://hml.driva.duckdns.org
#   API_BASE=https://api-hml.driva.duckdns.org/v1
#   PROJECT=default
#   SLUG=e2e-24-publicacao
#
...
# removível: o Chrome headless (pidfile) e um --user-data-dir temporário (nunca
# um fixo — perfil fixo é a causa de meio diagnóstico errado de render).
# NENHUMA mudança de código-fonte, NENHUM build.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
HERE="$ROOT/docs/24-publicacao-versionamento"
...
echo ""
echo "${b}E2E do item 24 na tela do hml → evidencias/rodada_$ROUND/${x}"
...
if [ "$STATUS" -eq 0 ]; then
  ...
  echo "${d}Confira as imagens pelo README.md — comece pela tabela dos dois modos de falha.${x}"
else
