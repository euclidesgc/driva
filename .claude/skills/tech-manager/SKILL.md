...
**Depois.** A bateria de unit/widget **fecha cada fase** (`escrever-testes`), não o item — não a empurre para o fim. A sequência final encolheu para: gate CISO → **E2E só se admitido** (os três critérios do `CLAUDE.md`; default é roteiro manual curto, não script) → dev atesta → `final_report.md` → gate CISO → docs vivas (`manter-docs-vivas`) → PR final.

**O E2E é exceção justificada, e o formato padrão é roteiro manual curto** _(decisão do dono, 2026-08-20)_. Admita-o só pelos três critérios do `CLAUDE.md` — integração real que nenhum fake reproduz, comportamento que só existe em runtime real, modo de falha sem golden que alcance — e escreva `docs/NN-<nome>/e2e_roteiro.md`: passos numerados que o humano executa, com print só do que a automação não prova. O par `e2e_hml.sh` + `e2e_shots.sh`/`e2e_drive.mjs` (contrato por API e prints headless por CDP) continua disponível e tem modelos no repo, mas agora é **escolha registrada no plano**, não o default. Onde houver E2E, ele roda **contra homologação**, nunca `localhost` — lição do item 9g. Evidências por rodada em `evidencias/rodada_MM/`; problema encontrado → o time corrige ou ajusta o script → próxima rodada.
