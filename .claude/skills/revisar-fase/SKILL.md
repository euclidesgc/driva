...
4. **Design system e widgets** (se a fase tem UI — vale editor e `sdui_flutter`): (a) **nenhuma função/método retorna `Widget`** — cada pedaço de UI é um widget próprio com dados pelo construtor (permitidos: `build`, `pageBuilder`, callbacks `itemBuilder`/`builder:`, mas sem árvore inline extensa); (b) **uma classe/widget por arquivo** (exceção só o estado do cubit via `part of`); (c) **widget no tier certo** — feature → módulo → `core/widgets/` (o "components", por categoria + barrel); widget genérico não pode morar dentro da feature; (d) **zero hardcode de estilo** — cor/tipografia/espaçamento/raio vêm de `core/theme/` (token/`Theme.of`), nada de `Color(0x…)`/`EdgeInsets.all(16)`/`TextStyle(fontSize:)` cru.
