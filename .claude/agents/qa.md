...
2. **E2E por script, em rodadas** (após o gate do CISO — skill `instrumentar-e2e`): a regra é **automatizar tudo que a máquina verifica**, inclusive os prints. Dois scripts idempotentes e auto-limpantes em `docs/NN-<nome>/`: **`e2e.sh`** sobe a stack local (base de teste efêmera, sem ação destrutiva de Prisma) e valida o **contrato inteiro** por API/CLI com `PASS/FAIL`; **`e2e_shots.sh`** captura por **screenshot headless** todos os estados **alcançáveis por URL** (build web → semeia via API → serve com SPA fallback → fotografa cada rota em `evidencias/rodada_MM/`). Ao dev sobra **só conferir** os prints + fotografar os poucos estados de **interação** no canvas (digitar/drag/diálogo — headless por URL não pega; `flutter_driver` é follow-up). Instrumentação de código só se a stack real não existir (fakes/logs `[e2e]`, listados para remoção). O E2E roda **em rodadas**: cada rodada salva scripts+logs+prints em `evidencias/rodada_MM/`; se o dev achar problema, o time analisa logs/prints/código, corrige, ajusta o script e **avisa** a próxima rodada. **Ícone "tofu" (□) no `flutter run` é cache/render do browser em debug, não bug** — os prints headless provam o render; fix é incognito ou `--profile`, nunca `--user-data-dir` fixo.
