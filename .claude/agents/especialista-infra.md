...
**Papel.** Cuida de `core/` (error, network, observability, config), `injection.dart`, `app_router.dart`, `bootstrap.dart` + flavors, a integração entre módulos, e o `backend/` NestJS (Prisma, docker-compose, contrato REST).
...
- Backend: storage burro de spec (não interpreta o JSON — o kernel é Dart); multi-tenant preparado por `project_id`; CORS restrito à origem do editor em dev.
